...
class MultiTouchListener implements OnTouchListener {
    ...
    MultiTouchListener(View deleteView, RelativeLayout parentView,
                       ImageView photoEditImageView, OnPhotoStickerSDKListener onPhotoStickerSDKListener) {
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        switch (action & event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                onMultiTouchListener.onFocusViewListener(view);
                view.bringToFront();
                ...
                if (mCurrentCancelX == mPrevRawX || mCurrentCancelY == mPrevRawY) {
                    if (view instanceof TextView) {
                        ...
                        if (onPhotoStickerSDKListener != null) {
                            onPhotoStickerSDKListener.onEditTextChangeListener(
                                    (TextView)view, ((TextView) view).getText().toString(), ((TextView) view).getCurrentTextColor(), ((TextView)view).getTypeface());
                        }
                    }
                }
                break;
    ...
    private void firePhotoEditorSDKListener(View view, boolean isStart) {
        if (view instanceof TextView) {
            if (onMultiTouchListener != null) {
                if (onPhotoStickerSDKListener != null) {
                    if (isStart)
                        onPhotoStickerSDKListener.onStartViewChangeListener(ViewType.TEXT);
                    else
                        onPhotoStickerSDKListener.onStopViewChangeListener(ViewType.TEXT);
                }
            } else {
                if (onPhotoStickerSDKListener != null) {
                    if (isStart)
                        onPhotoStickerSDKListener.onStartViewChangeListener(ViewType.EMOJI);
                    else
                        onPhotoStickerSDKListener.onStopViewChangeListener(ViewType.EMOJI);
                }
            }
        } else {
            if (onPhotoStickerSDKListener != null) {
                if (isStart)
                    onPhotoStickerSDKListener.onStartViewChangeListener(ViewType.IMAGE);
                else
                    onPhotoStickerSDKListener.onStopViewChangeListener(ViewType.IMAGE);
            }
        }
    }
    ...
    interface OnMultiTouchListener {
        void onEditTextClickListener(String text, int colorCode, Typeface font);
        void onRemoveViewListener(View removedView);
        void onFocusViewListener(View focousedView);
    }
}