...
public class EmoticonFragment extends Fragment implements EmoticonAdapter.OnImageClickListener {
...
//            stickerBitmaps.add(decodeSampledBitmapFromResource(stickerActivity.getResources(), images.getResourceId(i, -1), 120, 120));
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        EmoticonAdapter adapter = new EmoticonAdapter(stickerActivity, stickerBitmaps);
    ...
    public Bitmap decodeSampledBitmapFromAssets(int resId) {
        String strfileName = String.format("emots/%d.png", resId + 1);
        Bitmap ret=null;
        try {
    ...
    public Bitmap decodeSampledBitmapFromOBB(int resId) {
        String strfileName = String.format("assets/emots/%d.png", resId + 1);
        Bitmap ret=null;
        try {
//            ret = BitmapFactory.decodeStream(getContext().getAssets().open(strfileName));
    ...
    public void onImageClickListener(Bitmap image, int nImageIndex) {
        stickerActivity.addEmoticon(image, nImageIndex);
    }
}
