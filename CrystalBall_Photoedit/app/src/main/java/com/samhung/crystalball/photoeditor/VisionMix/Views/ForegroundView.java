...
public class ForegroundView extends SurfaceView implements SurfaceHolder.Callback{
    ...
    private float hardnessValue = 50;
...
//        public Path path = new Path();
    ...
    public ForegroundView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs);
...
//        setZOrderOnTop(true);
    ...
    public void setBrushHardness(float hardness) {this.hardnessValue = hardness;}
    public float getBrushHardness() {return hardnessValue; }
...
//        invalidate();
    ...
    RectF m_clipRect = null;
    public void setClipRect(int left, int top, int wid, int hei) {
        if(m_clipRect == null) m_clipRect = new RectF();
        tempCanvas = new Canvas(tempBitmap);
        m_clipRect.left = left;
        m_clipRect.top = top;
        m_clipRect.right = left +wid;
        m_clipRect.bottom = top+hei;
        tempCanvas.clipRect(m_clipRect);
        RefreshView();
    }
...
//        Log.d("-----draw", ""+(ee-dd));
        ...
        if (!drawPath.isEmpty()) {
            ...
            drawPaint.setXfermode(xferModeEraser);
            float brushSize = eraserSize;// * scale;
            float hardness = brushSize / 100 * hardnessValue;
            if(hardness!=0)
                drawPaint.setMaskFilter(new BlurMaskFilter(hardness, BlurMaskFilter.Blur.NORMAL));
            else
                drawPaint.setMaskFilter(null);
        ...
        if(m_clipRect == null)
            m_clipRect = new RectF(0, 0, getWidth(), getHeight());
        tempCanvas.clipRect(m_clipRect);
        canvas.drawBitmap(tempBitmap, 0, 0, copyingPaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                ForegroundView.StrokeInfo si = new ForegroundView.StrokeInfo();

                matrix.getValues(si.matrix_values);
                si.points.addAll(ppsList);
                si.strokeWidth = eraserSize / scale;
                si.hardness = hardnessValue ;/// scale;
                pathList.add(si);
                drawPath.reset();
                break;
            }
