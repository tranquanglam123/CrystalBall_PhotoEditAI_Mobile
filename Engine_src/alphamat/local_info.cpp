...
#pragma omp parallel for(default)
	...
	for (j = win_size; j < nCols - win_size; j++)
	{
		for (i = win_size; i < nRows - win_size; i++)
		{
			...
			Mat win = img.rowRange(i - win_size, i + win_size + 1);
			win = win.colRange(j - win_size, j + win_size + 1);
			Mat win_ravel = Mat::zeros(9, 3, CV_32F);  // doubt ??
			float sum1 = 0;
			float sum2 = 0;
			float sum3 = 0;
			...
			for (int q = 0; q < win_size * 2 + 1; q++)
			{
				for (int p = 0; p < win_size * 2 + 1; p++)
				{
					win_ravel.at<float>(c, 0) = win.at<cv::Vec3b>(p, q)[0] / 255.0;
					win_ravel.at<float>(c, 1) = win.at<cv::Vec3b>(p, q)[1] / 255.0;
					win_ravel.at<float>(c, 2) = win.at<cv::Vec3b>(p, q)[2] / 255.0;
					sum1 += win.at<cv::Vec3b>(p, q)[0] / 255.0;
					sum2 += win.at<cv::Vec3b>(p, q)[1] / 255.0;
					sum3 += win.at<cv::Vec3b>(p, q)[2] / 255.0;
					c++;
			...
			win = win_ravel;
			Mat win_mean = Mat::zeros(1, 3, CV_32F);
			win_mean.at<float>(0, 0) = sum1 / num_win;
			win_mean.at<float>(0, 1) = sum2 / num_win;
			win_mean.at<float>(0, 2) = sum3 / num_win;

			// calculate the covariance matrix
			Mat covariance = (win.t() * win / num_win) - (win_mean.t() * win_mean);

			Mat I = Mat::eye(img.channels(), img.channels(), CV_32F);
			Mat I1 = (covariance + (eps / num_win) * I);
			Mat I1_inv = I1.inv();
			...
			for (int q = 0; q < num_win; q++)
			{
				for (int p = 0; p < num_win; p++)
				{
					triplets.push_back(T(neighInd[p], neighInd[q], vals.at<float>(p, q)));
				}
			}
		}
	}
	...
	std::vector<T> tsp;
	SparseMatrix<float> W(N, N), Wsp(N, N);
	W.setFromTriplets(triplets.begin(), triplets.end());

	SparseMatrix<float> Wt = W.transpose();
	SparseMatrix<float> Ws = Wt + W;
	W = Ws;
	...
	SparseMatrix<float> Wlt = Wl.transpose();

	for (int k = 0; k < Wlt.outerSize(); ++k)
	{
		float sumarr = 0;
		for (SparseMatrix<float>::InnerIterator it(Wlt, k); it; ++it)
			sumarr += it.value();
		td.push_back(T(k, k, sumarr));
	}
