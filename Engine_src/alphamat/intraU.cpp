...
static
void generateFVectorIntraUDouble(my_vector_of_vectors_t_double& samples, Mat& img, Mat& tmap, std::vector<int>& orig_ind)
{
    int nRows = img.rows;
    int nCols = img.cols;
    int unk_count = 0;
...
static
void generateFVectorIntraUFloat(my_vector_of_vectors_t_float& samples, Mat& img, Mat& tmap, std::vector<int>& orig_ind)
{
	int nRows = img.rows;
	int nCols = img.cols;
	int unk_count = 0;
	...
	for (i = 0; i < nRows; ++i)
	{
		for (j = 0; j < nCols; ++j)
		{
			...
			if (pix == 128)  // collection of unknown pixels samples
			{
				...
				samples[c1][1] = img.at<cv::Vec3b>(i, j)[1] / 255.0;
				samples[c1][2] = img.at<cv::Vec3b>(i, j)[2] / 255.0;
				samples[c1][3] = (float(i + 1) / nRows) / 20;
				samples[c1][4] = (float(j + 1) / nCols) / 20;
				orig_ind[c1] = i * nCols + j;
				c1++;
			}
		}
...
static
void kdtree_intraU_double(Mat& img, Mat& tmap, my_vector_of_vectors_t_double& indm, my_vector_of_vectors_t_double& samples, std::vector<int>& orig_ind)
{
    // Generate feature vectors for intra U:
    generateFVectorIntraUDouble(samples, img, tmap, orig_ind);
...
static
void kdtree_intraU_float(Mat& img, Mat& tmap, my_vector_of_vectors_t_float& indm, my_vector_of_vectors_t_float& samples, std::vector<int>& orig_ind)
{
	// Generate feature vectors for intra U:
	generateFVectorIntraUFloat(samples, img, tmap, orig_ind);
...
static
double l1norm_double(std::vector<double>& x, std::vector<double>& y)
{
    double sum = 0;
...
static
double l1norm_float(std::vector<float>& x, std::vector<float>& y)
{
	float sum = 0;
	for (int i = 0; i < ALPHAMAT_DIM; i++)
		sum += abs(x[i] - y[i]);
...
static
void intraU_double(Mat& img, my_vector_of_vectors_t_double& indm, my_vector_of_vectors_t_double& samples,
	std::vector<int>& orig_ind, SparseMatrix<double>& Wuu, SparseMatrix<double>& Duu)
{
    ...
    char szTmp[256];
    sprintf(szTmp, "math.inst.atd");
    CV_LOG_INFO(NULL, szTmp);
	...
	my_vector_of_vectors_t_double weights;
	...
	for (i = 0; i < n; i++)
	{
		...
		for (j = 0; j < num_nbr; j++)
		{
			...
			weight = max(1 - l1norm_double(samples[i], samples[j]), 0.0);
...
static
void intraU_float(Mat& img, my_vector_of_vectors_t_float& indm, my_vector_of_vectors_t_float& samples,
	std::vector<int>& orig_ind, SparseMatrix<float>& Wuu, SparseMatrix<float>& Duu)
{
	// input: indm, samples
	int n = indm.size();  // num of unknown samples
	CV_LOG_INFO(NULL, "ALPHAMAT: num of unknown samples, n : " << n);
	...
	my_vector_of_vectors_t_float weights;
	...
	for (i = 0; i < n; i++)
	{
		...
		for (j = 0; j < num_nbr; j++)
		{
			nbr_ind = indm[i][j];
			int cMaj_nbr_j = findColMajorInd(orig_ind[nbr_ind], img.rows, img.cols);
			weight = max(1 - l1norm_float(samples[i], samples[j]), 0.0);
...
{
    my_vector_of_vectors_t_double samples, indm;
    std::vector<int> orig_ind;

    kdtree_intraU_double(image, tmap, indm, samples, orig_ind);
    intraU_double(image, indm, samples, orig_ind, Wuu, Duu);
    CV_LOG_INFO(NULL, "ALPHAMAT: Intra U Done");
}

void UU_float(Mat& image, Mat& tmap, SparseMatrix<float>& Wuu, SparseMatrix<float>& Duu)
{
	my_vector_of_vectors_t_float samples, indm;
	std::vector<int> orig_ind;

	kdtree_intraU_float(image, tmap, indm, samples, orig_ind);
	intraU_float(image, indm, samples, orig_ind, Wuu, Duu);
	CV_LOG_INFO(NULL, "ALPHAMAT: Intra U Done");
}
