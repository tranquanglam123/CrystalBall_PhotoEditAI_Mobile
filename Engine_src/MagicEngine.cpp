...
namespace CrystalBall {
	...
	void MagicEngine::init(const string &model_path) {
		...
		int nRet = m_netSegmentation.load_param(model_file.c_str());
		nRet +=m_netSegmentation.load_model(trained_file.c_str());
		if(nRet > 0) bInitSucced = false;
		else bInitSucced = true;
	}
...
//		}
	...
	//Pre-process trimap
	void MagicEngine::preprocTrimapImage(Mat trimapImage, bool bMono) {
		int nRows = trimapImage.rows;
		int nCols = trimapImage.cols;
		uchar midValue = bMono?0:128;
        int n255_counts = 0;
        ...
        if(n255_counts < 100) {
            Mat tmp = Mat(trimapImage, cv::Rect(trimapImage.cols/2-trimapImage.cols/10, trimapImage.rows/2-trimapImage.rows/10, trimapImage.cols/5, trimapImage.rows/5));
            tmp |=255;
        }
	}
...
// 		matImage.convertTo(matImage, CV_8UC1);
	...
	cv::Mat MagicEngine::mattingImageInformationFlowFloatOnly(cv::Mat &srcImage,
																	 cv::Mat &trimapImage) {
	...
	cv::Mat MagicEngine::mattingImageGuideFilterOnly(cv::Mat &srcImage,
																	 cv::Mat &matImage) {
...
//		Mat newImage = srcImage.clone();
	...
	cv::Mat MagicEngine::make128Border(cv::Mat &srcImage, bool bEmpty) {
		Mat dilation, border, preproc;
        if(bEmpty) {
            srcImage *= 0;
            Mat img = Mat(srcImage, cv::Rect(60,60, 100, 100));
            img |= 255;
        }
		Dilation(srcImage, dilation, 7);

		cv::bitwise_or(srcImage, dilation, border);
		border = border / 2;
		cv::bitwise_or(srcImage, border, preproc);
	...
	cv::Mat MagicEngine::mattingImageForEffectImage(cv::Mat &srcImage,
		cv::Mat &trimapImage, cv::Point &position) {
		...
		/////////////
		Mat dilation, border, preproc;
		Dilation(trimapImage, dilation, 7);

		cv::bitwise_or(trimapImage, dilation, border);
		border = border / 2;
		cv::bitwise_or(trimapImage, border, preproc);
...
#define MAX_GUIDE_FILTER_SIZE        1200
        ...
        sprintf(str, "%d %d %d %d", x, y, pic_wid, pic_hei);
        __android_log_write(ANDROID_LOG_DEBUG, "---guide filter---",  str);
...
//		imwrite("D:/matImageDefault2.png", matImage);
...
#ifndef WIN32
		sprintf(str, "%d   %d", retImg.cols, retImg.rows);
		__android_log_write(ANDROID_LOG_DEBUG, "---last---", str);
#endif
	...
	cv::Mat MagicEngine::effectImageColorOnYou(cv::Mat &srcImage,
		...
		cv::Mat matImage = mattingImageInformationFlowFloatGuideFilter(srcImage, trimapImage);
...
// 		imwrite("D:/effect.png", filterImg);
	...
	cv::Mat MagicEngine::effectImageRomantic(int cx, int cy, cv::Mat& srcImage1, cv::Mat& srcImage2, Point2f pt1[4], Point2f pt2[4])
	{
		Mat retImg(cy, cx, srcImage1.type());
...
//		Size warped_image_size2 = Size(frmImage.cols, frmImage.rows);
		Size warped_image_size1 = Size(cx, cy);
		Size warped_image_size2 = Size(cx, cy);
